function Global:Convert-CSIMacAddressToBytes {
...
function Global:Convert-CSIBytesToIPv4 {
...
function Global:New-CSIDHCPInformPacket {
...
param(
    ...
    $macBytes = Convert-CSIMacAddressToBytes $MacAddress
...
function Global:Get-CSIDHCPPacketOptions {
...
function Global:Get-CSINetworkNeighborHint {
...
function Global:Invoke-CSIDHCPInformProbe {
...
param(
    ...
    try {
        ...
        $packet = New-CSIDHCPInformPacket `
        ...
        while((Get-Date) -lt $stopAt){
            ...
            try {
                ...
                $options = Get-CSIDHCPPacketOptions -Packet $response
                $serverId = $null

                if($options.ContainsKey(54)){
                    $serverId = Convert-CSIBytesToIPv4 $options[54]
                }
                ...
                if($options.ContainsKey(3)){
                    $router = Convert-CSIBytesToIPv4 $options[3]
                }

                if($options.ContainsKey(6)){
                    for($i = 0; $i -lt $options[6].Count; $i += 4){
                        if(($i + 3) -lt $options[6].Count){
                            $dnsServers += Convert-CSIBytesToIPv4 ([byte[]]$options[6][$i..($i + 3)])
                        }
                    }
                }
...
function Global:Get-CSIDHCPClientAdapters {
...
function Global:Get-CSIIpconfigDHCPServers {
...
function Global:Resolve-CSITSharkPath {

    if(Get-Command Resolve-CSIExternalTool -ErrorAction SilentlyContinue){
        $tool = Resolve-CSIExternalTool -Id "TShark"
    ...
    if($CSIPaths -and $CSIPaths.Root){
        $root = Join-Path $CSIPaths.Root "ExternalTools"
    }
...
function Global:Get-CSITSharkInterfaces {
...
function Global:Convert-CSITSharkDHCPLine {
...
function Global:Invoke-CSITSharkDHCPCapture {
...
param(
    ...
    $interfaces = @(Get-CSITSharkInterfaces -TSharkPath $tshark)
    ...
    if($InterfaceNumber -gt 0){
        ...
        Write-Host ""
        $selection = Read-CSIInput "Capture interface number (blank for first listed interface)" -AllowEmpty
    ...
    $outputFile = Join-Path $env:TEMP ("CSI-TShark-DHCP-{0}.out" -f ([guid]::NewGuid().ToString("N")))
    $errorFile = Join-Path $env:TEMP ("CSI-TShark-DHCP-{0}.err" -f ([guid]::NewGuid().ToString("N")))
    ...
    try {
        ...
        foreach($line in $lines){
            ...
            $result = Convert-CSITSharkDHCPLine ("$line|$($selected.Name)")
...
param(
    ...
    if(!$KnownServer -and !$NoPrompt){
        $knownInput = Read-CSIInput "Known/approved DHCP server IPs (comma separated, blank to trust current lease server)" -AllowEmpty
    ...
    $adapters = @(Get-CSIDHCPClientAdapters)
    $findings = @()

    if(!$UseTShark -and !$NoPrompt){
        $tsharkChoice = Read-CSIInput "Run Wireshark CLI/TShark DHCP capture? Y/N (blank = Yes)" -AllowEmpty
    ...
    if($UseTShark -and !$RenewLease -and !$NoPrompt){
        $renewChoice = Read-CSIInput "Run ipconfig /renew during capture to force DHCP traffic? Y/N (blank = No)" -AllowEmpty
    ...
    foreach($adapter in $adapters){
        ...
        $probeResults += Invoke-CSIDHCPInformProbe -Adapter $adapter -TimeoutSeconds $TimeoutSeconds
    ...
    if($UseTShark){
        $tsharkResults = @(Invoke-CSITSharkDHCPCapture -DurationSeconds ([Math]::Max($CaptureSeconds, $TimeoutSeconds)) -InterfaceNumber $CaptureInterface -RenewLease:$RenewLease)
    }

    $ipconfigServers = @(Get-CSIIpconfigDHCPServers)
    ...
    foreach($adapter in $adapters){
        ...
        if($adapter.DHCPServer){
            $macHint = Get-CSINetworkNeighborHint -IPAddress $adapter.DHCPServer
        }
