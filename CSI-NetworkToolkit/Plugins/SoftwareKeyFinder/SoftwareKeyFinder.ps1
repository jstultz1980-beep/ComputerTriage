function Global:ConvertTo-CSIProductKey {
...
function Global:Get-CSIWindowsLicenseKeys {
    ...
    foreach($registryPath in @(
        ...
        try {
            ...
            $decoded = ConvertTo-CSIProductKey -DigitalProductId $value
...
function Global:Get-CSIOfficeLicenseKeys {
    $results = New-Object System.Collections.ArrayList
    $paths = @(
    ...
    foreach($basePath in $paths){
        ...
        foreach($registrationKey in @(Get-ChildItem -Path $basePath -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '\\Registration\\[^\\]+$' })){
            try {
                ...
                $decoded = ConvertTo-CSIProductKey -DigitalProductId $digitalId
...
function Global:Get-CSIMicrosoftActivationInventory {
    try {
        return @(
...
function Global:ConvertTo-CSIKeyFinderHtml {
    param([object]$Value)
    return [System.Net.WebUtility]::HtmlEncode([string]$Value)
}

function Global:Export-CSISoftwareKeyReport {
    param(
    ...
    $path = Join-Path $CSIPaths.Exports "software-key-report-$safeComputer-$stamp.html"
    $keyRows = if(@($RecoveredKeys).Count -gt 0){
        (@($RecoveredKeys) | ForEach-Object {
            "<tr><td>$(ConvertTo-CSIKeyFinderHtml $_.Product)</td><td class='key'>$(ConvertTo-CSIKeyFinderHtml $_.Key)</td><td>$(ConvertTo-CSIKeyFinderHtml $_.Source)</td><td>$(ConvertTo-CSIKeyFinderHtml $_.Note)</td></tr>"
        }) -join "`n"
    }
    else{
        "<tr><td colspan='4' class='empty'>No recoverable full Windows or legacy Office key was found. This is normal for many digitally licensed, Microsoft 365, volume-licensed, and subscription installations.</td></tr>"
    }
    $activationRows = if(@($ActivationInventory).Count -gt 0){
        (@($ActivationInventory) | ForEach-Object {
            "<tr><td>$(ConvertTo-CSIKeyFinderHtml $_.Name)</td><td>$(ConvertTo-CSIKeyFinderHtml $_.LicenseStatus)</td><td>$(ConvertTo-CSIKeyFinderHtml $_.PartialProductKey)</td><td>$(ConvertTo-CSIKeyFinderHtml $_.Description)</td></tr>"
        }) -join "`n"
    }
    else{
...
</style></head><body><div class="top"><h1>Network Toolkit Software Key Report</h1><p>$(ConvertTo-CSIKeyFinderHtml $env:COMPUTERNAME) | Generated $(Get-Date -Format "g")</p></div><main class="wrap"><div class="notice"><strong>Confidential licensing data:</strong> handle this report as customer-sensitive information. It attempts to recover Windows firmware/registry keys and legacy Microsoft Office keys only. It does not attempt to recover browser passwords, Wi-Fi passwords, API tokens, or third-party application secrets.</div><section class="section"><h2>Recoverable Product Keys</h2><table><thead><tr><th style="width:22%">Product</th><th style="width:24%">Key</th><th style="width:24%">Source</th><th>Notes</th></tr></thead><tbody>$keyRows</tbody></table></section><section class="section"><h2>Windows and Office Activation Inventory</h2><table><thead><tr><th style="width:35%">Product</th><th style="width:12%">Status</th><th style="width:15%">Last Five</th><th>Description</th></tr></thead><tbody>$activationRows</tbody></table></section><div class="footer">Generated by Network Toolkit. Do not retain this report longer than needed for approved licensing support.</div></main></body></html>
...
function Global:Invoke-SoftwareKeyFinder {
    ...
    Write-Host ""
    Write-Host "This performs a read-only local licensing check for Windows and legacy Microsoft Office keys." -ForegroundColor Yellow
    Write-Host "It does not retrieve passwords, Wi-Fi keys, browser data, API tokens, or third-party secrets." -ForegroundColor DarkGray
    Write-Host ""

    $keys = @(Get-CSIWindowsLicenseKeys) + @(Get-CSIOfficeLicenseKeys)
    $activation = @(Get-CSIMicrosoftActivationInventory)
    $reportPath = Export-CSISoftwareKeyReport -RecoveredKeys $keys -ActivationInventory $activation
    ...
    if($keys.Count -gt 0){
        ...
        Write-Host "No recoverable full Windows or legacy Office key was found." -ForegroundColor Yellow
    }
...
Register-CSICommand `
    ...
    -Category "Software" `
    -Description "Read-only recovery of Windows firmware/registry keys and legacy Microsoft Office keys, with a confidential HTML report." `
    -Order 640
