...
function Build-WindowsUpdatePage {
    ...
    Update-GUIWindowsUpdateHealthIndicator
}

function Get-GUIWifiSignalInfo {
    $info = [ordered]@{
        Available = $false
        Connected = $false
        Signal = $null
    ...
    try {
        $output = @(netsh.exe wlan show interfaces 2>$null | ForEach-Object { [string]$_ })
        ...
        foreach($line in $output){
            if($line -match '^\s*State\s*:\s*(.+)$'){
                ...
                $info.Connected = ($state -match 'connected')
            }
        ...
        if($info.Connected -and $null -ne $info.Signal){
            $info.Detail = if($info.SSID){"$($info.SSID): $($info.Signal)%"}else{"$($info.Signal)%"}
        }
        elseif($info.Available){
            $info.Detail = "Wi-Fi not connected"
        }
    }
    catch {
        $info.Detail = "Wi-Fi unavailable"
    }
    finally {
        # Status probes should never leak external command exit codes into the GUI launcher.
...
function Update-GUIWifiIndicators {
    ...
    if($script:WifiStatusLabel -and !$script:WifiStatusLabel.IsDisposed){
        ...
        $script:WifiStatusLabel.ForeColor = $color
        $script:WifiStatusLabel.ToolTipText = $info.Detail
    }
...
function Get-GUIWindowsUpdateServiceHealth {
    $names = @("wuauserv","UsoSvc","bits")
    $states = @()
    foreach($name in $names){
        try {
            $service = Get-Service -Name $name -ErrorAction Stop
            $states += [pscustomobject]@{Name=$name;Status=[string]$service.Status}
        }
        catch {
            $states += [pscustomobject]@{Name=$name;Status="Missing"}
        }
    }
...
function Update-GUIWindowsUpdateHealthIndicator {
    if(!$script:WUHealthLabel -or $script:WUHealthLabel.IsDisposed){
        return
    }
...
function New-GUIChatGPTBundle {
    ...
    try {
        ...
        @(
            ...
            "9. Use collection-manifest.json first to understand which collector sections completed, failed, or were skipped. Do not penalize a computer for skipped role-specific checks such as DC or DFSR checks when they are not applicable."
            "10. Use event-timeline-focused.* for reboot, update, service-control, crash/WER, disk/NTFS, driver-framework, Hyper-V, and network event correlations. Cite exact provider, event ID, time, and file name for each significant event."
            "11. Use driver-focus.* and security-product-inventory.json to identify likely driver, endpoint security, VPN, backup, or filter-driver involvement. Name the exact product, driver, service, process, or binding only when it appears in the evidence."
            "12. Use network-stack.json plus ipconfig-all.txt, route-print.txt, netcfg-bindings.txt, and network-bindings.csv to identify adapter, DNS, gateway, route, VPN, and binding concerns."
            "13. Use crash-dump-inventory.* to distinguish retained crash evidence from crash events. Do not claim a dump was collected unless the inventory proves a dump exists."
            "14. Identify likely root-cause candidates only when supported by the evidence, and describe the relevant event timeline."
            "15. Return a downloadable JSON file named NetworkToolkit-AI-Analysis-$computerName.json that strictly conforms to AI-ANALYSIS-SCHEMA.json in this ZIP. Do not return HTML."
            "16. The JSON must include the executive summary, risk level, findings, evidence citations, event timeline, root-cause candidates, remediation checklist, and missing evidence. Also show a concise summary in chat."
            ""
            "## Analysis Rules"
            ""
...
function Build-TriagePage {
    ...
    $TriageToolGrid.Visible = $false

    $runGroup = New-Object System.Windows.Forms.GroupBox
    $runGroup.Text = "1. Collect"
    $runGroup.Dock = "Fill"
    $runGroup.Margin = New-Object System.Windows.Forms.Padding(0,0,8,0)
    $layout.Controls.Add($runGroup,0,1)
    ...
    $reviewGroup.Dock = "Fill"
    $reviewGroup.Margin = New-Object System.Windows.Forms.Padding(4,0,4,0)
    $layout.Controls.Add($reviewGroup,1,1)
    ...
    $submitGroup.Dock = "Fill"
    $submitGroup.Margin = New-Object System.Windows.Forms.Padding(8,0,0,0)
    $layout.Controls.Add($submitGroup,2,1)
...
function Build-Form {
    ...
    $GUIBusyProgress.ToolTipText = "Shown while the toolkit is running a background action."
