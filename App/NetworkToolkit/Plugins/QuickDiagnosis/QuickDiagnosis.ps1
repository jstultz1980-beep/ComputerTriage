function Global:Convert-NTKHtml {
...
function Global:Export-NTKQuickDiagnosisHtml {
...
param(
        ...
        @(for($problemIndex = 0; $problemIndex -lt $problems.Count; $problemIndex++){
            ...
            if($deepDive){
                $candidateDriverHtml = if($deepDive.CandidateDriver){
                    "<div class='wide'><span>Candidate Driver</span><p>$(Convert-NTKHtml $deepDive.CandidateDriver)</p></div>"
                }
                else{
                    ""
                }

                $deepDiveHtml = "<div class='finding-detail'><div class='finding-subhead'>Deeper Investigation</div><div class='finding-body investigation-grid'><div><span>Confidence</span><p>$(Convert-NTKHtml $deepDive.Confidence)</p></div><div><span>Evidence</span><p>$(Convert-NTKHtml $deepDive.Evidence)</p></div><div><span>Impact</span><p>$(Convert-NTKHtml $deepDive.WhyItMatters)</p></div><div><span>Best Next Action</span><p>$(Convert-NTKHtml $deepDive.RecommendedAction)</p></div>$candidateDriverHtml</div></div>"
            ...
            "<div class='finding-card'><div class='finding-head'><span class='badge $($problem.Status.ToLower())'>$(Convert-NTKHtml $problem.Status)</span><strong>$(Convert-NTKHtml $problem.Area) - $(Convert-NTKHtml $problem.Check)</strong></div><div class='finding-body problem-grid'><div><span>Supporting Evidence</span><p>$(Convert-NTKHtml ($supportingEvidence -join "`n`n"))</p></div><div><span>Recommended Next Step</span><p>$(Convert-NTKHtml $recommendedAction)</p></div></div>$deepDiveHtml</div>"
        }) -join "`n"
    }
    else{
    ...
    $checkRows = ($checks | ForEach-Object {
        ...
        if($status -eq "OK"){
            "<tr><td><span class='badge ok'>OK</span></td><td>$(Convert-NTKHtml $task)</td><td class='oktext'>Error free</td><td></td></tr>"
        }
        elseif($status -eq "Info"){
            "<tr><td><span class='badge info'>Info</span></td><td>$(Convert-NTKHtml $task)</td><td>$(Convert-NTKHtml $_.Detail)</td><td>$(Convert-NTKHtml $_.Tip)</td></tr>"
        }
        else{
            $evidence = "$($_.Detail) $($_.Explanation)".Trim()
            "<tr><td><span class='badge $statusClass'>$(Convert-NTKHtml $status)</span></td><td>$(Convert-NTKHtml $task)</td><td>$(Convert-NTKHtml $evidence)</td><td>$(Convert-NTKHtml $_.Tip)</td></tr>"
        }
    }) -join "`n"

    $diskRows = if($fingerprint -and $fingerprint.Disks){
        (@($fingerprint.Disks) | ForEach-Object {
            "<tr><td>$(Convert-NTKHtml $_.DeviceID)</td><td>$(Convert-NTKHtml $_.VolumeName)</td><td>$(Convert-NTKHtml $_.SizeGB)</td><td>$(Convert-NTKHtml $_.FreeGB)</td></tr>"
        }) -join "`n"
    }
    else{
    ...
    $adapterRows = if($fingerprint -and $fingerprint.NetworkAdapters){
        (@($fingerprint.NetworkAdapters) | ForEach-Object {
            "<tr><td>$(Convert-NTKHtml $_.Interface)</td><td>$(Convert-NTKHtml $_.IPv4)</td><td>$(Convert-NTKHtml $_.Gateway)</td><td>$(Convert-NTKHtml $_.DNS)</td></tr>"
        }) -join "`n"
    }
    else{
        ...
        $problems |
            ForEach-Object {
                [pscustomobject]@{
                    ...
                    Action = [string]$_.Tip
                    Steps = @(Get-NTKQuickRemediationSteps -Finding $_)
                }
            } |
            Group-Object Key |
    ...
    $remediationChecklist = if($remediationItems.Count -gt 0){
        (@($remediationItems | ForEach-Object {
            $stepHtml = if(@($_.Steps).Count -gt 0){
                "<ol class='remediation-steps'>" + ((@($_.Steps) | ForEach-Object { "<li>$(Convert-NTKHtml $_)</li>" }) -join "") + "</ol>"
            }
            else{ "" }
            "<li class='remediation-item $($_.Status.ToLower())'><label><input type='checkbox'><span class='badge $($_.Status.ToLower())'>$(Convert-NTKHtml $_.Status)</span><strong>$(Convert-NTKHtml $_.Title)</strong></label><div class='remediation-evidence'><span>Why this was flagged</span>$(Convert-NTKHtml $_.Evidence)</div><p><strong>Recommended action:</strong> $(Convert-NTKHtml $_.Action)</p>$stepHtml</li>"
        }) -join "`n")
    }
    else{
...
<head>
<meta charset="utf-8">
<title>Network Toolkit Quick Diagnosis - $(Convert-NTKHtml $Report.ComputerName)</title>
<style>
...
<div class="topbar">
  ...
  <p>$(Convert-NTKHtml $Report.ComputerName) | $(Convert-NTKHtml $Report.CollectedAt) | Target: $(Convert-NTKHtml $Report.Target)</p>
</div>
<div class="wrap">
  <div class="summary">
    <div class="card status-card $overallClass">
      ...
      <p class="status-note">$(Convert-NTKHtml $overallNote)</p>
    </div>
    <div class="card metric"><span>Critical</span><strong>$(Convert-NTKHtml $Report.Summary.Critical)</strong></div>
    <div class="card metric"><span>Warning</span><strong>$(Convert-NTKHtml $Report.Summary.Warning)</strong></div>
    <div class="card metric"><span>Info</span><strong>$(Convert-NTKHtml $Report.Summary.Info)</strong></div>
    <div class="card metric"><span>OK</span><strong>$(Convert-NTKHtml $Report.Summary.OK)</strong></div>
  </div>
  ...
  <div class="section grid">
    <div class="card">
      ...
      <div class="kv">
        <div>User</div><div>$(Convert-NTKHtml $Report.UserName)</div>
        <div>Domain</div><div>$(Convert-NTKHtml $(if($fingerprint){$fingerprint.Domain}else{""}))</div>
        <div>Model</div><div>$(Convert-NTKHtml $(if($fingerprint){"$($fingerprint.Manufacturer) $($fingerprint.Model)"}else{""}))</div>
        <div>OS</div><div>$(Convert-NTKHtml $(if($fingerprint){"$($fingerprint.OS) $($fingerprint.OSVersion) Build $($fingerprint.OSBuild)"}else{""}))</div>
        <div>Uptime Days</div><div>$(Convert-NTKHtml $(if($fingerprint){$fingerprint.UptimeDays}else{""}))</div>
        <div>Memory GB</div><div>$(Convert-NTKHtml $(if($fingerprint){$fingerprint.MemoryGB}else{""}))</div>
        <div>Pending Reboot</div><div>$(Convert-NTKHtml $(if($fingerprint){$fingerprint.PendingReboot.Pending}else{""}))</div>
      </div>
    </div>
    <div class="card">
      <h2>Repair Disposition</h2>
      <p>$(Convert-NTKHtml $Report.RepairDisposition)</p>
    </div>
  </div>
...
function Global:Add-NTKQuickFinding {
...
function Global:Get-NTKQuickRemediationSteps {
...
function Global:Get-NTKQuickSysinternalsPath {
...
param(
    ...
    if($NTKPaths -and $NTKPaths.Root){
        $roots += Join-Path $NTKPaths.Root "ExternalTools\Sysinternals"
    }
...
function Global:Invoke-NTKQuickExternalCapture {
...
param(
    ...
    try {
        if(Get-Command Set-NTKSysinternalsEulaAccepted -ErrorAction SilentlyContinue){
            Set-NTKSysinternalsEulaAccepted -Path $FilePath
        }

        if(Get-Command Add-NTKSysinternalsEulaArgument -ErrorAction SilentlyContinue){
            $Arguments = @(Add-NTKSysinternalsEulaArgument -Path $FilePath -Arguments $Arguments)
    ...
    if($cleanArguments.Count -gt 0){
        if(Get-Command Join-NTKCommandLine -ErrorAction SilentlyContinue){
            $psi.Arguments = Join-NTKCommandLine -Parts $cleanArguments
        }
        else{
...
function Global:Get-NTKQuickSysinternalsEvidence {
    ...
    $autorunsc = Get-NTKQuickSysinternalsPath -Names @("autorunsc64.exe","autorunsc.exe")
    if($autorunsc){
        $capture = Invoke-NTKQuickExternalCapture -FilePath $autorunsc -Arguments @("-accepteula","-nobanner","-m","-c") -TimeoutSeconds 10
        $rows = @()
    ...
    $coreinfo = Get-NTKQuickSysinternalsPath -Names @("coreinfo64.exe","coreinfo.exe")
    if($coreinfo){
        $capture = Invoke-NTKQuickExternalCapture -FilePath $coreinfo -Arguments @("-accepteula","-nobanner") -TimeoutSeconds 8
    ...
    $psloggedon = Get-NTKQuickSysinternalsPath -Names @("PsLoggedon64.exe","PsLoggedon.exe","psloggedon64.exe","psloggedon.exe")
    if($psloggedon){
        $capture = Invoke-NTKQuickExternalCapture -FilePath $psloggedon -Arguments @("-accepteula","-nobanner","-l") -TimeoutSeconds 8
    ...
    $logonsessions = Get-NTKQuickSysinternalsPath -Names @("logonsessions64.exe","logonsessions.exe")
    if($logonsessions){
        $capture = Invoke-NTKQuickExternalCapture -FilePath $logonsessions -Arguments @("-accepteula","-nobanner") -TimeoutSeconds 8
...
function Global:Add-NTKQuickSysinternalsFindings {
...
param(
    ...
    $evidence = Get-NTKQuickSysinternalsEvidence

    function Format-NTKQuickAutorunsRow {
    param([object]$Row)
    ...
    if($evidence.Autoruns){
        ...
        if($autoruns.TimedOut){
            Add-NTKQuickFinding $Findings "Startup" "Autoruns" "Info" "Autorunsc did not finish within the quick diagnosis timeout. Partial startup evidence may still be available in the Computer Profile." "Use the Autoruns button for deeper startup and persistence review if symptoms point to startup, malware, or logon slowness."
        }
        elseif($autoruns.MissingFileCount -gt 0){
            $detail = (($autoruns.MissingFiles | Select-Object -First 5 | ForEach-Object { Format-NTKQuickAutorunsRow $_ }) -join "; ")
            Add-NTKQuickFinding $Findings "Startup" "Autoruns Missing Files" "Warning" "$($autoruns.MissingFileCount) definite startup/persistence file path(s) are missing. Exact affected entries: $detail" "In Autoruns, locate each exact entry/location listed above. Confirm its application was intentionally removed or the path is truly absent, then disable the entry first and test before deleting it."
        }
        elseif($autoruns.CommandSyntaxReviewCount -gt 0){
            $detail = (($autoruns.CommandSyntaxReviews | Select-Object -First 5 | ForEach-Object { Format-NTKQuickAutorunsRow $_ }) -join "; ")
            Add-NTKQuickFinding $Findings "Startup" "Autoruns Command Syntax Review" "Info" "Autoruns could not resolve $($autoruns.CommandSyntaxReviewCount) command-style startup target(s), but it did not find a definite missing executable. Exact entry: $detail" "Review the exact location and launch command in Autoruns. The reported target may be a built-in cmd.exe command rather than a missing file. Keep it unless the alternate-shell or startup command is unexpected for this computer."
        }
        elseif($autoruns.UnsignedOrUnverifiedCount -gt 0){
            $detail = (($autoruns.UnsignedOrUnverified | Select-Object -First 5 | ForEach-Object { Format-NTKQuickAutorunsRow $_ }) -join "; ")
            Add-NTKQuickFinding $Findings "Startup" "Autoruns Verification" "Warning" "$($autoruns.UnsignedOrUnverifiedCount) non-Microsoft startup/persistence entries are unsigned or unverified. $detail" "Open Autoruns and Sigcheck. Verify the publisher and path before disabling anything; replace suspicious software from a trusted vendor source."
        }
        else{
            Add-NTKQuickFinding $Findings "Startup" "Autoruns" "OK" "Autorunsc checked $($autoruns.EntriesChecked) startup/persistence entries without missing paths or unverified non-Microsoft entries in the quick scan."
        }
    }
    else{
        Add-NTKQuickFinding $Findings "Startup" "Autoruns" "Info" "Autorunsc was not found in Sysinternals." "Install or restore Sysinternals Autoruns if startup/persistence evidence should be included in Quick Diagnosis."
    }

    if($evidence.Coreinfo){
        $coreinfo = $evidence.Coreinfo
        if($coreinfo.TimedOut){
            Add-NTKQuickFinding $Findings "Hardware" "Coreinfo" "Info" "Coreinfo did not finish within the quick diagnosis timeout." "Use Coreinfo manually if CPU virtualization or hypervisor feature details are needed."
        }
        elseif($coreinfo.HypervisorMentioned){
            Add-NTKQuickFinding $Findings "Virtualization" "Coreinfo" "Info" "Coreinfo output indicates hypervisor/virtualization context. $((@($coreinfo.VirtualizationLines) | Select-Object -First 4) -join '; ')" "For VM hardware, validate disk, CPU, memory, and drivers at the hypervisor layer when guest-only evidence is inconclusive."
        }
        else{
            Add-NTKQuickFinding $Findings "Hardware" "Coreinfo" "OK" "Coreinfo ran successfully. No hypervisor-specific warning was detected in the quick scan."
        }
    }
    ...
    if($evidence.LoggedOnUsers){
        ...
        if($users.Count -gt 0){
            Add-NTKQuickFinding $Findings "User Sessions" "Logged On Users" "Info" "PsLoggedOn found local logged-on user context: $($users -join '; ')" "Use this to confirm whether the active user context matches the reported issue. For profile or lock issues, check the listed user sessions before rebooting."
        }
        elseif($evidence.LoggedOnUsers.TimedOut){
            Add-NTKQuickFinding $Findings "User Sessions" "Logged On Users" "Info" "PsLoggedOn did not finish within the quick diagnosis timeout." "Use PsLoggedOn manually if remote/local session context matters."
        }
        else{
            Add-NTKQuickFinding $Findings "User Sessions" "Logged On Users" "OK" "PsLoggedOn did not return extra local logged-on users."
        }
    }
...
function Global:Get-NTKQuickRemediationTip {
...
function Global:Get-NTKQuickPlainExplanation {
...
function Global:Add-NTKQuickTipsToFindings {
...
param([object[]]$Findings)
    ...
    foreach($finding in @($Findings)){
        ...
        if($finding.PSObject.Properties["Tip"] -and $finding.Tip){
            ...
            $tip = Get-NTKQuickRemediationTip -Finding $finding
        }
        ...
        [pscustomobject]@{
            ...
            Explanation = Get-NTKQuickPlainExplanation -Finding $finding
            Tip         = $tip
        }
...
function Global:Invoke-NTKQuickDismCheckHealth {

    $findings = @()

    if(!(Get-Command dism.exe -ErrorAction SilentlyContinue)){
        Add-NTKQuickFinding ([ref]$findings) "System Image" "DISM CheckHealth" "Info" "dism.exe was not found." "Verify Windows servicing tools are available on this machine."
        return $findings
    }
    ...
    try {
        ...
        if($LASTEXITCODE -eq 0 -and $text -match "No component store corruption detected"){
            Add-NTKQuickFinding ([ref]$findings) "System Image" "DISM CheckHealth" "OK" "No component store corruption detected."
        }
        elseif($text -match "repairable|corruption|component store"){
            Add-NTKQuickFinding ([ref]$findings) "System Image" "DISM CheckHealth" "Warning" ($text.Substring(0,[math]::Min(240,$text.Length))) "Run Windows Health Diagnostics > DISM/SFC Repair Path if repair is approved, then reboot if repairs occur and rerun Quick Diagnosis."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "System Image" "DISM CheckHealth" "Warning" "DISM exited with code $LASTEXITCODE. $($text.Substring(0,[math]::Min(200,$text.Length)))" "Rerun Quick Diagnosis elevated. If it still fails, inspect Windows servicing health."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "System Image" "DISM CheckHealth" "Warning" $_.Exception.Message "Rerun Quick Diagnosis elevated. If it still fails, inspect Windows servicing health."
    }
...
function Global:Test-NTKQuickVirtualMachine {
...
function Global:Get-NTKQuickDriverDate {
    ...
    try {
        $drivers = @(
            Get-CimInstance Win32_PnPSignedDriver -ErrorAction Stop |
                ...
                ForEach-Object {
                    $driverDate = Get-NTKQuickDriverDate -Driver $_
                    [pscustomobject]@{
...
function Global:Add-NTKQuickExtendedComputerChecks {
...
param(
    ...
    $isVM = Test-NTKQuickVirtualMachine -Fingerprint $fingerprint
    $driverInventory = Get-NTKQuickDriverInventory
    ...
    if($isVM){
        Add-NTKQuickFinding $Findings "Virtualization" "Machine Type" "Info" "This appears to be a virtual machine: $($fingerprint.Manufacturer) $($fingerprint.Model)." "Do not rely on guest-only SMART or temperature tools for final hardware health. Check the hypervisor host/storage layer when disk or hardware symptoms are suspected."
    }

    try {
        $video = @(Get-CimInstance Win32_VideoController -ErrorAction Stop)
        if($video.Count -eq 0){
            Add-NTKQuickFinding $Findings "Hardware" "Display Adapter" "Info" "No display adapters were returned by Windows inventory." "If this is a VM or server, this may be normal. If this is a workstation, open Device Manager and GPU-Z/HWiNFO."
        }
        else{
            ...
            if($badVideo.Count -gt 0){
                Add-NTKQuickFinding $Findings "Hardware" "Display Adapter" "Warning" "$($badVideo.Count) display adapter(s) have a Device Manager error. $displaySummary" "Open Device Manager and GPU-Z or HWiNFO. Reinstall or update the display driver if this is a physical workstation."
            }
            elseif(!$isVM -and $genericPhysicalVideo.Count -gt 0){
                ...
                Add-NTKQuickFinding $Findings "Hardware" "Display Adapter" "Warning" "Physical workstation appears to be using a generic Microsoft display driver. $driverText" "Install the OEM display driver for this exact computer model, or the GPU vendor driver if the OEM package is unavailable."
            }
            else{
                Add-NTKQuickFinding $Findings "Hardware" "Display Adapter" "OK" $displaySummary
            }
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Hardware" "Display Adapter" "Info" "Unable to inspect display adapters: $($_.Exception.Message)" "Open Device Manager manually if graphics symptoms exist."
    }

    try {
        $bios = Get-CimInstance Win32_BIOS -ErrorAction Stop
        $releaseDate = Get-NTKQuickDriverDate -Driver ([pscustomobject]@{ DriverDate = $bios.ReleaseDate })
        if($releaseDate){
            $ageYears = [math]::Round(((Get-Date) - $releaseDate).TotalDays / 365,1)
            if($ageYears -ge 5){
                Add-NTKQuickFinding $Findings "Firmware" "BIOS Age" "Warning" "BIOS $($bios.SMBIOSBIOSVersion) was released $ageYears years ago." "Check the vendor support page or OEM update utility. Update firmware only during an approved maintenance window."
            }
            else{
                Add-NTKQuickFinding $Findings "Firmware" "BIOS Age" "OK" "BIOS $($bios.SMBIOSBIOSVersion) release date appears about $ageYears years old."
            }
        }
        else{
            Add-NTKQuickFinding $Findings "Firmware" "BIOS Age" "Info" "BIOS release date was not available." "Check vendor firmware manually if hardware symptoms exist."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Firmware" "BIOS Age" "Info" "Unable to inspect BIOS information." "Check vendor firmware manually if hardware symptoms exist."
    }

    try {
        $batteries = @(Get-CimInstance Win32_Battery -ErrorAction Stop)
        if($batteries.Count -eq 0){
            Add-NTKQuickFinding $Findings "Power" "Battery Status" "Info" "No battery was detected. This is normal for desktops, servers, and many virtual machines."
        }
        else{
            ...
            if($bad.Count -gt 0){
                Add-NTKQuickFinding $Findings "Power" "Battery Status" "Warning" $summary "Run powercfg /batteryreport or the OEM diagnostics tool and review battery health."
            }
            else{
                Add-NTKQuickFinding $Findings "Power" "Battery Status" "OK" $summary
            }
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Power" "Battery Status" "Info" "Unable to inspect battery status." "If this is a laptop with power symptoms, run powercfg /batteryreport."
    }
    ...
    try {
        if(Get-Command Get-Tpm -ErrorAction SilentlyContinue){
            ...
            if($tpm.TpmPresent -and $tpm.TpmReady){
                Add-NTKQuickFinding $Findings "Security" "TPM" "OK" "TPM is present and ready."
            }
            else{
                $status = if($isVM){"Info"}else{"Warning"}
                Add-NTKQuickFinding $Findings "Security" "TPM" $status "TPM present: $($tpm.TpmPresent); TPM ready: $($tpm.TpmReady)." "Enable TPM/vTPM if BitLocker, Windows Hello, or device security requires it."
            }
        }
        else{
            Add-NTKQuickFinding $Findings "Security" "TPM" "Info" "TPM cmdlet is not available." "Check tpm.msc if TPM status matters."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Security" "TPM" "Info" "Unable to inspect TPM status." "Check tpm.msc if TPM status matters."
    }
    ...
    try {
        if(Get-Command Confirm-SecureBootUEFI -ErrorAction SilentlyContinue){
            ...
            if($secureBoot){
                Add-NTKQuickFinding $Findings "Security" "Secure Boot" "OK" "Secure Boot is enabled."
            }
            else{
                $status = if($isVM){"Info"}else{"Warning"}
                Add-NTKQuickFinding $Findings "Security" "Secure Boot" $status "Secure Boot is disabled." "Enable Secure Boot if required by the workstation security baseline."
            }
        }
        else{
            Add-NTKQuickFinding $Findings "Security" "Secure Boot" "Info" "Secure Boot check is not available in this environment." "This is common on legacy BIOS systems and some VMs."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Security" "Secure Boot" "Info" "Secure Boot could not be checked: $($_.Exception.Message)" "This is common on legacy BIOS systems and some VMs."
    }
    ...
    try {
        if(Get-Command Get-BitLockerVolume -ErrorAction SilentlyContinue){
            ...
            if($volumes.Count -eq 0){
                Add-NTKQuickFinding $Findings "Security" "BitLocker" "Info" "No BitLocker-capable fixed volumes were returned." "Check manage-bde -status if encryption status matters."
            }
            elseif($unprotected.Count -gt 0){
                Add-NTKQuickFinding $Findings "Security" "BitLocker" "Warning" "$($unprotected.Count) fixed volume(s) are not protected by BitLocker." "Enable or resume BitLocker if this system should be encrypted."
            }
            else{
                Add-NTKQuickFinding $Findings "Security" "BitLocker" "OK" "Fixed volumes returned by BitLocker are protected."
            }
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Security" "BitLocker" "Info" "Unable to inspect BitLocker status." "Check manage-bde -status if encryption status matters."
    }
    ...
    try {
        ...
        if($license.Count -gt 0 -and $license[0].LicenseStatus -eq 1){
            Add-NTKQuickFinding $Findings "Licensing" "Windows Activation" "OK" "Windows reports an activated license state."
        }
        elseif($license.Count -gt 0){
            Add-NTKQuickFinding $Findings "Licensing" "Windows Activation" "Warning" "Windows license status code: $($license[0].LicenseStatus)." "Check Activation settings, KMS reachability, or the license channel."
        }
        else{
            Add-NTKQuickFinding $Findings "Licensing" "Windows Activation" "Info" "No Windows licensing product with a partial key was returned." "Check Activation settings manually if the user sees activation warnings."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Licensing" "Windows Activation" "Info" "Unable to inspect Windows activation status." "Check Activation settings manually if the user sees activation warnings."
    }
    ...
    try {
        if(Get-Command Get-PhysicalDisk -ErrorAction SilentlyContinue){
            ...
            if($physicalDisks.Count -eq 0){
                Add-NTKQuickFinding $Findings "Hardware" "Storage Visibility" "Info" "Windows did not return physical disks through Get-PhysicalDisk." "On a VM this is expected. For physical hardware, use CrystalDiskInfo, HWiNFO, or vendor storage tools."
            }
            elseif($isVM){
                Add-NTKQuickFinding $Findings "Hardware" "Storage Visibility" "Info" "Physical disk data is visible, but this appears to be a VM. Guest disk health may not reflect host storage health." "Check the hypervisor or storage platform if disk symptoms exist."
            }
        }
    }
    catch {
        $status = if($isVM){"Info"}else{"Warning"}
        Add-NTKQuickFinding $Findings "Hardware" "Storage Visibility" $status "Physical disk inventory could not be read: $($_.Exception.Message)" "On VMs this is often normal. On physical systems, use CrystalDiskInfo/HWiNFO or vendor tools."
    }
    ...
    try {
        ...
        if($oldDrivers.Count -gt 0){
            ...
            Add-NTKQuickFinding $Findings "Drivers" "Driver Age" "Info" "$($oldDrivers.Count) non-Microsoft important driver(s) are older than five years. $detail" "Only update drivers when symptoms, vendor guidance, or security requirements justify it."
        }
        else{
            Add-NTKQuickFinding $Findings "Drivers" "Driver Age" "OK" "No non-Microsoft display, network, storage, chipset, audio, USB, Bluetooth, or printer drivers older than five years were found."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Drivers" "Driver Age" "Info" "Unable to inspect driver age." "Use Device Manager or vendor update tools if driver age is suspect."
    }
    ...
    try {
        ...
        if($unsigned.Count -gt 0){
            ...
            Add-NTKQuickFinding $Findings "Drivers" "Unsigned Drivers" "Warning" "$($unsigned.Count) non-Microsoft driver(s) are unsigned or signature status is unknown. $detail" "Review these in Device Manager or with Sigcheck. Replace suspicious or unsupported drivers with vendor-signed versions."
        }
        else{
            Add-NTKQuickFinding $Findings "Drivers" "Unsigned Drivers" "OK" "No unsigned non-Microsoft drivers were found in the important driver classes."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Drivers" "Unsigned Drivers" "Info" "Unable to inspect driver signature status." "Use Sigcheck or Device Manager if driver trust is suspect."
    }
    ...
    try {
        ...
        if($recentDrivers.Count -gt 0){
            ...
            Add-NTKQuickFinding $Findings "Drivers" "Recently Dated Drivers" "Info" "$($recentDrivers.Count) non-Microsoft important driver(s) have driver dates within the last 45 days. $detail" "If symptoms started recently, compare these against recent updates or vendor installs."
        }
        else{
            Add-NTKQuickFinding $Findings "Drivers" "Recently Dated Drivers" "OK" "No non-Microsoft important drivers with driver dates in the last 45 days were found."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Drivers" "Recently Dated Drivers" "Info" "Unable to inspect recently dated drivers." "If symptoms began after an update, check Device Manager driver dates manually."
    }
    ...
    try {
        ...
        if($driverEvents.Count -gt 0){
            ...
            Add-NTKQuickFinding $Findings "Drivers" "Recent Driver Events" "Warning" "$($driverEvents.Count) driver/device warning or error event(s) found in the last 14 days. $detail" "Open Event Log Triage and Device Manager. Look for repeated provider, device, or driver names."
        }
        else{
            Add-NTKQuickFinding $Findings "Drivers" "Recent Driver Events" "OK" "No recent driver/device warning or error events were found in common driver event providers."
        }
    }
    catch {
        Add-NTKQuickFinding $Findings "Drivers" "Recent Driver Events" "Info" "Unable to inspect recent driver events." "Open Event Viewer System log and check Kernel-PnP and DriverFrameworks providers."
    }

}

function Global:Get-NTKQuickStatusChecks {

param([pscustomobject]$BaseReport)

    $findings = @()

    $findings += Invoke-NTKQuickDismCheckHealth
    Add-NTKQuickExtendedComputerChecks ([ref]$findings) -BaseReport $BaseReport
    Add-NTKQuickSysinternalsFindings ([ref]$findings) -BaseReport $BaseReport | Out-Null
    ...
    try {
        ...
        if($critical -gt 0){
            Add-NTKQuickFinding ([ref]$findings) "Event Logs" "Last 24 Hours" "Critical" "$critical critical and $errors error events found." "Open Windows Health Diagnostics > Event Log Triage and review the newest critical events first."
        }
        elseif($errors -gt 10){
            Add-NTKQuickFinding ([ref]$findings) "Event Logs" "Last 24 Hours" "Warning" "$errors error events found." "Open Windows Health Diagnostics > Event Log Triage and group errors by provider."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Event Logs" "Last 24 Hours" "OK" "$errors error events and no critical events found."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Event Logs" "Last 24 Hours" "Info" "Unable to read recent event logs." "Rerun elevated or open Event Viewer manually."
    }
    ...
    try {
        ...
        if($stoppedAuto.Count -gt 0){
            Add-NTKQuickFinding ([ref]$findings) "Services" "Stopped Automatic Services" "Warning" (($stoppedAuto | Select-Object -First 8 -ExpandProperty Name) -join ",") "Open Windows Health Diagnostics > Service Health and verify whether each listed service should be running."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Services" "Stopped Automatic Services" "OK" "No unexpected stopped automatic services found."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Services" "Stopped Automatic Services" "Info" "Unable to evaluate service state." "Rerun elevated or inspect services.msc manually."
    }
    ...
    try {
        ...
        if(Get-Command Get-MpComputerStatus -ErrorAction SilentlyContinue){
            ...
            if($issues){
                Add-NTKQuickFinding ([ref]$findings) "Security" "Defender" "Warning" ($issues -join "; ") "Open Windows Security or Defender Security Check and verify AV/signature status."
            }
            else{
                Add-NTKQuickFinding ([ref]$findings) "Security" "Defender" "OK" "Defender and real-time protection appear enabled."
            }

        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Security" "Defender" "Info" "Defender cmdlets are not available." "Confirm the active antivirus/security platform manually."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Security" "Defender" "Info" "Unable to evaluate Defender status." "Confirm the active antivirus/security platform manually."
    }
    ...
    try {
        ...
        if(Get-Command Get-PhysicalDisk -ErrorAction SilentlyContinue){
            ...
            if($badDisks.Count -gt 0){
                Add-NTKQuickFinding ([ref]$findings) "Disk" "Physical Disk Health" "Warning" (($badDisks | ForEach-Object {"$($_.FriendlyName) $($_.HealthStatus) $($_.OperationalStatus)"}) -join "; ") "Open CrystalDiskInfo or HWiNFO and inspect SMART/drive health immediately."
            }
            else{
                Add-NTKQuickFinding ([ref]$findings) "Disk" "Physical Disk Health" "OK" "Physical disks report healthy."
            }

        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Disk" "Physical Disk Health" "Info" "Get-PhysicalDisk is not available." "Use CrystalDiskInfo or vendor tools to verify disk health."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Disk" "Physical Disk Health" "Info" "Unable to evaluate physical disk health." "Use CrystalDiskInfo or vendor tools to verify disk health."
    }
    ...
    try {
        ...
        if(Get-Command Get-NTKHardwareDeviceProblems -ErrorAction SilentlyContinue){
        ...
        if($deviceProblems.Count -gt 0){
            ...
            $detail = @(
                $deviceProblems |
                    ...
                    ForEach-Object {
                        $driver = Get-NTKQuickDriverForDevice -DeviceProblem $_ -DriverInventory $driverInventory
            ...
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "Device Manager" "Warning" "$($deviceProblems.Count) problem device(s): $detail" "Open Hardware Health Diagnostics and Device Manager. Fix driver, firmware, cabling, or missing-device problems."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "Device Manager" "OK" "No present Device Manager problem devices found."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Hardware" "Device Manager" "Info" "Unable to evaluate Device Manager problem devices." "Open Device Manager manually or rerun elevated."
    }
    ...
    try {
        ...
        if($smartFailures.Count -gt 0){
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "SMART Failure Prediction" "Critical" "$($smartFailures.Count) disk device(s) report predicted failure." "Back up data immediately and inspect disk health with CrystalDiskInfo or vendor diagnostics."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "SMART Failure Prediction" "OK" "No storage-driver predicted disk failures reported."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Hardware" "SMART Failure Prediction" "Info" "SMART failure prediction was not available from Windows." "Use CrystalDiskInfo, HWiNFO, or vendor diagnostics if disk symptoms exist."
    }
    ...
    try {
        ...
        if($wheaEvents.Count -gt 0){
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "WHEA Events" "Critical" "$($wheaEvents.Count) WHEA warning/error event(s) found in the last 14 days." "Open Hardware Health Diagnostics. Update BIOS/firmware/chipset/storage drivers and run OEM diagnostics."
        }
        elseif($hardwareEvents.Count -gt 0){
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "Hardware Driver Events" "Warning" "$($hardwareEvents.Count) hardware/driver warning or error event(s) found in the last 14 days." "Open Hardware Health Diagnostics and review the newest provider events."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Hardware" "Hardware Events" "OK" "No recent common hardware/driver warning, error, or critical events found."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Hardware" "Hardware Events" "Info" "Unable to evaluate recent hardware events." "Open Event Viewer System log and check WHEA, Kernel-PnP, disk, and storage providers."
    }

    try {

        $dumps = @()
        if(Get-Command Get-NTKDumpFiles -ErrorAction SilentlyContinue){
            $dumps = @(Get-NTKDumpFiles | Select-Object -First 5)
        }
        ...
        if($crashEvents.Count -gt 0){
            if($dumps.Count -gt 0){
                Add-NTKQuickFinding ([ref]$findings) "Crashes" "Recent WER/Bugcheck Events" "Warning" "$($crashEvents.Count) crash or WER events found in the last 14 days. $($dumps.Count) dump file(s) are also available for analysis." "Run Minidump Collector And Analyzer, then review the collected dump with BlueScreenView or WinDbg Preview. Correlate the dump time with the listed crash events."
            }
            else{
                Add-NTKQuickFinding ([ref]$findings) "Crashes" "Recent WER/Application Crash Events" "Warning" "$($crashEvents.Count) crash or WER events found in the last 14 days, but Windows has not retained any .dmp files in the common dump locations." "Open Crash Event Summary or Reliability Monitor to identify the repeating application/provider. Minidump Collector will export the event evidence, but BlueScreenView cannot analyze a crash until Windows produces a dump file."
            }
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Crashes" "Recent WER/Bugcheck Events" "OK" "No recent WER or bugcheck events found."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Crashes" "Recent WER/Bugcheck Events" "Info" "Unable to evaluate crash events." "Rerun elevated or inspect Reliability Monitor/Event Viewer."
    }
    ...
    try {
        ...
        if(Get-Command Get-NTKDumpFiles -ErrorAction SilentlyContinue){
            if($dumps.Count -gt 0){
                Add-NTKQuickFinding ([ref]$findings) "Crashes" "Dump Files" "Warning" "$($dumps.Count) recent dump files found. Latest: $($dumps[0].FullName)" "Run Minidump Collector And Analyzer and review the newest dump."
            }
            else{
                Add-NTKQuickFinding ([ref]$findings) "Crashes" "Dump Files" "Info" "No .dmp files were found in common dump locations. This is expected for application-only crashes or when Windows dump creation/retention is disabled."
            }
        }
    ...
    try {
        ...
        if($freeMemoryPct -lt 10){
            Add-NTKQuickFinding ([ref]$findings) "Resources" "Available Memory" "Warning" "$freeMemoryPct% physical memory free." "Open Resource Hotspots, Process Explorer, or RAMMap to identify memory pressure."
        }
        else{
            Add-NTKQuickFinding ([ref]$findings) "Resources" "Available Memory" "OK" "$freeMemoryPct% physical memory free."
        }

    }
    catch {
        Add-NTKQuickFinding ([ref]$findings) "Resources" "Available Memory" "Info" "Unable to evaluate memory pressure." "Open Task Manager or Resource Hotspots manually."
    }
    ...
    try {
        ...
        if($printers.Count -gt 0){
            ...
            if(!$spooler -or $spooler.Status -ne "Running"){
                Add-NTKQuickFinding ([ref]$findings) "Print" "Spooler" "Warning" "Printers exist but spooler is $($spooler.Status)." "Open Print Queue Tools and check spooler state and stuck jobs."
            }
            else{
                Add-NTKQuickFinding ([ref]$findings) "Print" "Spooler" "OK" "Spooler running with $($printers.Count) printer(s)."
            }
...
function Global:New-NTKQuickDeepDive {
...
function Global:Format-NTKQuickDriverCandidate {
...
function Global:Get-NTKQuickDeepDives {
...
param([pscustomobject]$Report)
    ...
    if($needsDriverCandidateLookup){
        $driverCandidates = @(Get-NTKQuickWindowsUpdateDriverCandidates)
    }
    ...
    foreach($finding in $findings){
        ...
        if($area -eq "Hardware" -and $check -eq "Display Adapter"){
            ...
            $candidate = if($driver.Count -gt 0){ Find-NTKQuickDriverCandidate -Driver $driver[0] -Candidates $driverCandidates }else{$null}
            $candidateText = Format-NTKQuickDriverCandidate -Candidate $candidate -AllCandidates $driverCandidates
            ...
            $deepDives += New-NTKQuickDeepDive `
                -Area "Hardware" `
        ...
        elseif($area -eq "Hardware" -and $check -eq "Device Manager"){
            ...
            if($problemDrivers.Count -gt 0){
                $candidate = Find-NTKQuickDriverCandidate -Driver $problemDrivers[0] -Candidates $driverCandidates
                $candidateText = Format-NTKQuickDriverCandidate -Candidate $candidate -AllCandidates $driverCandidates
            }
        ...
        elseif($area -eq "Drivers" -and $check -eq "Recent Driver Events"){
            $deepDives += New-NTKQuickDeepDive `
                ...
                -CandidateDriver (Format-NTKQuickDriverCandidate -Candidate $null -AllCandidates $driverCandidates)
        }
...
function Global:Add-NTKQuickStatusChecksToReport {

param([pscustomobject]$Report)

    $quickChecks = @(Get-NTKQuickStatusChecks -BaseReport $Report)
    $combinedHealth = Add-NTKQuickTipsToFindings (@($Report.Health) + $quickChecks)
    $problems = @($combinedHealth | Where-Object {$_.Status -in @("Critical","Warning")})
    $summary = Get-NTKTriageSummary -Health $combinedHealth

    $Report.Health = $combinedHealth
    $Report.Problems = $problems
    $Report.Summary = $summary
    $Report | Add-Member -MemberType NoteProperty -Name DeepDives -Value @(Get-NTKQuickDeepDives -Report $Report) -Force
...
function Global:Export-NTKQuickIssueEvidence {
param([pscustomobject]$Report)
    ...
    $path = Join-Path $NTKPaths.Exports ("issue-evidence-{0}-{1}.json" -f $computerName,(Get-Date -Format 'yyyyMMdd-HHmmss'))
    ...
    if($report){
        ...
        try {
            if($report.Fingerprint -and (Get-Command Save-NTKComputerFingerprint -ErrorAction SilentlyContinue)){
                ...
                $fingerprintExports = Save-NTKComputerFingerprint -Fingerprint $report.Fingerprint -OutputRoot $NTKPaths.Exports -Prefix "computer-profile" -Timestamped
            ...
            $htmlPath = Export-NTKQuickDiagnosisHtml -Report $report -OutputRoot $NTKPaths.Exports
            ...
            if(Get-Command Set-NTKComputerStateSection -ErrorAction SilentlyContinue){
                try {
                    ...
                    if($report.Fingerprint -and $report.FingerprintExports -and (Get-Command Export-NTKComputerFingerprintHtml -ErrorAction SilentlyContinue)){
                        if($report.FingerprintExports.DataJson -and (Test-Path $report.FingerprintExports.DataJson)){
                            $dataHtml = Export-NTKComputerFingerprintHtml -Fingerprint $report.Fingerprint -JsonPath $report.FingerprintExports.DataJson
                        ...
                        if($report.FingerprintExports.Json -and (Test-Path $report.FingerprintExports.Json)){
                            $exportHtml = Export-NTKComputerFingerprintHtml -Fingerprint $report.Fingerprint -JsonPath $report.FingerprintExports.Json
