function Global:Get-NTKReportFiles {
...
function Global:Open-NTKOutputFile {
...
param([string]$Path)
    ...
    if($extension -in ".html",".htm",".pdf"){
        Start-NTKToolProcess -FilePath $Path | Out-Null
    }
    else{
        Start-NTKToolProcess -FilePath "notepad.exe" -ArgumentList @("`"$Path`"") | Out-Null
    }

}

function Global:Get-NTKOutputLocations {

    $locations = @(
        [pscustomobject]@{
            Name = "Exports"
            Path = $NTKPaths.Exports
        ...
        [pscustomobject]@{
            Name = "Minidumps"
            Path = (Join-Path $NTKPaths.Data "MiniDumps")
            Notes = "Collected minidumps and crash-analysis output"
        }
        [pscustomobject]@{
            Name = "Print Queue Data"
            Path = (Join-Path $NTKPaths.Data "PrintQueues")
...
function Global:Invoke-ReportSelector {
    ...
    $reports = @(Get-NTKReportFiles)

    if($reports.Count -eq 0){

        Write-Host "No reports found." -ForegroundColor Yellow
        Write-Host "Reports are stored in:" $NTKPaths.Exports
        return
    ...
    $choice = Read-NTKInput "Select report"
    ...
    if($action -match "^(o|open)$"){
        ...
        Open-NTKOutputFile -Path $selected.FullName
...
function Global:Invoke-OpenToolkitOutputs {
    ...
    while($true){
        ...
        $locations = @(Get-NTKOutputLocations)
        ...
        $choice = Read-NTKInput "Select output location"
        ...
        if($choice -eq "0"){
            ...
            foreach($location in $locations | Where-Object { Test-Path $_.Path }){
                Open-NTKPath -Path $location.Path
            }
        ...
        Open-NTKPath -Path $selected.Path
        return
...
param(
    ...
    if(!$CIDR){
        $CIDR = Read-NTKInput "CIDR to scan for report"
    }
    ...
    if(!$OutputPath){
        ...
        $OutputPath = Join-Path $NTKPaths.Exports "network-report-$stamp.$extension"
