function Global:Get-NTKSubnetInfo {
...
param(
    ...
    if(!$CIDR){
        $CIDR = Read-NTKInput "CIDR network"
    }

    try {
        $subnet = Get-NTKSubnetInfo $CIDR
    }
    catch {
    ...
    if(!$CompareCIDR){
        $CompareCIDR = Read-NTKInput "Compare overlap with another CIDR (optional)" -AllowEmpty
    }
    ...
    if($CompareCIDR){
        ...
        try {
            ...
            $compare = Get-NTKSubnetInfo $CompareCIDR
