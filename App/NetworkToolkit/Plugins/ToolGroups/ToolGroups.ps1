function Global:Invoke-ConnectivityDiagnostics {
    ...
    while($true){
        ...
        $choice = Read-NTKInput "Select connectivity task"
        ...
        switch($choice){
            ...
            "6" { Invoke-NTKExternalTool -Id "TCPView" }
            "7" { Invoke-NTKExternalTool -Id "PsPing" }
            "8" { Invoke-NTKExternalTool -Id "Wireshark" }
            "9" { Invoke-NTKExternalTool -Id "NpcapInstaller" }
...
function Global:Invoke-InfrastructureServices {
    ...
    while($true){
        ...
        $choice = Read-NTKInput "Select infrastructure task"
...
function Global:Invoke-NetworkUtilities {
    ...
    while($true){
        ...
        $choice = Read-NTKInput "Select network utility"

        switch($choice){
            "1" { Invoke-SubnetCalculator }
            "2" { Invoke-WakeOnLANTool }
            "3" { Invoke-NTKExternalTool -Id "PuTTY" }
            "4" { Invoke-NTKExternalTool -Id "PsExec" }
            default { Write-Host "Invalid selection." -ForegroundColor Red }
        }
...
function Global:Invoke-SoftwareUtilities {
    ...
    while($true){
        ...
        $choice = Read-NTKInput "Select software utility"
