function Global:Get-NTKRemoteManagementEnableBlock {
...
function Global:Write-NTKRemoteManagementResults {
...
function Global:Get-NTKPsExecPath {

    $candidates = @()

    if($NTKPaths -and $NTKPaths.Root){
        $candidates += Join-Path $NTKPaths.Root "ExternalTools\Sysinternals\PsExec64.exe"
        $candidates += Join-Path $NTKPaths.Root "ExternalTools\Sysinternals\PsExec.exe"
        $candidates += Join-Path $NTKPaths.Root "ExternalTools\Sysinternals\psexec64.exe"
        $candidates += Join-Path $NTKPaths.Root "ExternalTools\Sysinternals\psexec.exe"
    }
...
function Global:Invoke-NTKEnableRemoteManagementWithPsExec {
...
param(
    ...
    $enableBlockText = (Get-NTKRemoteManagementEnableBlock).ToString()
    $command = @"
`$results = & {
$enableBlockText
}
...
function Global:Invoke-NTKEnableRemoteManagementTarget {
...
param(
    ...
    if($Method -eq "Local" -or $ComputerName -eq "." -or $ComputerName -eq $env:COMPUTERNAME){
        return @(& (Get-NTKRemoteManagementEnableBlock))
    }

    if($Method -in @("Auto","WinRM")){
        try {
            Test-WSMan -ComputerName $ComputerName -ErrorAction Stop | Out-Null
            return @(Invoke-Command -ComputerName $ComputerName -ScriptBlock (Get-NTKRemoteManagementEnableBlock) -ErrorAction Stop)
        }
        catch {
            if($Method -eq "WinRM"){
                return @([pscustomobject]@{
    ...
    return @(Invoke-NTKEnableRemoteManagementWithPsExec -ComputerName $ComputerName)
}

function Global:Get-NTKRemoteManagementReadinessBlock {
...
function Global:Invoke-NTKRemoteManagementReadinessTarget {

param([string]$ComputerName)

    if(!$ComputerName -or $ComputerName -eq "." -or $ComputerName -eq $env:COMPUTERNAME){
        return @(& (Get-NTKRemoteManagementReadinessBlock))
    }
    ...
    if($winrmOk){
        try {
            $remoteDetails = @(Invoke-Command -ComputerName $ComputerName -ScriptBlock (Get-NTKRemoteManagementReadinessBlock) -ErrorAction Stop)
...
function Global:Invoke-EnableRemoteManagement {
    ...
    if(Get-Command Test-NTKAdministrator -ErrorAction SilentlyContinue){
    ...
    $targetInput = Read-NTKInput "Target computer(s), comma separated. Blank = local computer" -AllowEmpty
    $targets = @()
    ...
    $methodInput = Read-NTKInput "Remote method [A=Auto, W=WinRM only, P=PsExec only] (default A)" -AllowEmpty
    ...
    if($targets.Count -gt 1 -or $targets[0] -ne $env:COMPUTERNAME){
        ...
        Write-Host ""
        $confirm = Read-NTKInput "Continue? Type YES" -AllowEmpty
        if($confirm -ne "YES"){
    ...
    foreach($target in $targets){
        ...
        $results = @(Invoke-NTKEnableRemoteManagementTarget -ComputerName $target -Method $targetMethod)
        $allResults += $results
        Write-NTKRemoteManagementResults -Results $results
        ...
        try {
            ...
            if(Get-Command Set-NTKComputerStateSection -ErrorAction SilentlyContinue){
                [void](Set-NTKComputerStateSection -SectionName "RemoteManagement" -Data $state -ComputerName $target -Source "Invoke-EnableRemoteManagement")
            }
        }
        catch {}
    }
...
function Global:Invoke-RemoteManagementReadiness {
    ...
    foreach($target in $targets){
        ...
        $results = @(Invoke-NTKRemoteManagementReadinessTarget -ComputerName $target)
        Write-NTKRemoteManagementResults -Results $results
        ...
        try {
            ...
            if(Get-Command Set-NTKComputerStateSection -ErrorAction SilentlyContinue){
                [void](Set-NTKComputerStateSection -SectionName "RemoteManagementReadiness" -Data $state -ComputerName $target -Source "Invoke-RemoteManagementReadiness")
            }
        }
        catch {}
    }
