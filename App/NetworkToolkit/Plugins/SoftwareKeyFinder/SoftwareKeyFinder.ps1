...
function Global:Invoke-SoftwareKeyFinder {
    param([switch]$Reveal,[switch]$ConfirmSensitiveAction,[string]$Reason='Interactive software licensing support')
    Clear-Host
    Write-Host ""
    ...
    $reportPath = Export-NTKSoftwareKeyReport -RecoveredKeys $keys -ActivationInventory $activation -Reveal:$Reveal -ConfirmSensitiveAction:$ConfirmSensitiveAction -Reason $Reason
