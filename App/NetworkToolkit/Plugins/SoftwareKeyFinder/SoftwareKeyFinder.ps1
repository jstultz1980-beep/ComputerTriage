...
function Global:Get-NTKApplicationLicenseEntries {
    param([int]$MaxDurationSeconds = 8)
    $results = New-Object System.Collections.ArrayList
    ...
    foreach($basePath in @(
        ...
        if($timer.Elapsed.TotalSeconds -ge $MaxDurationSeconds){ break }
        if(!(Test-Path $basePath)){
            continue
        }
        ...
        while($queue.Count -gt 0 -and $timer.Elapsed.TotalSeconds -lt $MaxDurationSeconds){
            $item = $queue.Dequeue()
            try {
...
function Global:ConvertTo-NTKMaskedSensitiveValue {
    param([object]$Value)
    $text=[string]$Value
    if([string]::IsNullOrEmpty($text)){return ''}
    $visible=[Math]::Min(5,$text.Length)
    return (('*' * [Math]::Max(4,$text.Length-$visible)) + $text.Substring($text.Length-$visible))
}
...
function Global:Export-NTKSoftwareKeyReport {
    param(
        ...
        [switch]$Reveal,
        [switch]$ConfirmSensitiveAction,
        [switch]$IncludedApplicationRegistration,
        [string]$Reason = ''
    )

    if($Reveal -and !$ConfirmSensitiveAction){ throw 'Unmasked export requires explicit -ConfirmSensitiveAction.' }
    ...
    $keyRows = if(@($RecoveredKeys).Count -gt 0){
        (@($RecoveredKeys) | ForEach-Object {
            $displayKey=if($Reveal){$_.Key}else{ConvertTo-NTKMaskedSensitiveValue $_.Key}
            "<tr><td>$(ConvertTo-NTKKeyFinderHtml $_.Product)</td><td class='key'>$(ConvertTo-NTKKeyFinderHtml $displayKey)</td><td>$(ConvertTo-NTKKeyFinderHtml $_.Source)</td><td>$(ConvertTo-NTKKeyFinderHtml $_.Note)</td></tr>"
        }) -join "`n"
    }
    else{
...
</style></head><body><div class="top"><h1>Network Toolkit Software Key Report</h1><p>$(ConvertTo-NTKKeyFinderHtml $env:COMPUTERNAME) | Generated $(Get-Date -Format "g")</p></div><main class="wrap"><div class="notice"><strong>Confidential licensing data:</strong> handle this report as customer-sensitive information. It checks Windows firmware/registry keys and legacy Microsoft Office entries$(if($IncludedApplicationRegistration){', plus a time-bounded scan of explicit local application registration values'}else{'. Broad application registration scanning was not requested'}). It does not attempt to recover browser passwords, Wi-Fi passwords, API tokens, or password/token-style registry values.</div><section class="section"><h2>Recoverable Product And Registration Entries</h2><table><thead><tr><th style="width:22%">Product</th><th style="width:24%">Key Or Registration Value</th><th style="width:24%">Source</th><th>Notes</th></tr></thead><tbody>$keyRows</tbody></table></section><section class="section"><h2>Microsoft Activation Inventory</h2><table><thead><tr><th style="width:35%">Product</th><th style="width:12%">Status</th><th style="width:15%">Last Five</th><th>Description</th></tr></thead><tbody>$activationRows</tbody></table></section><div class="footer">Generated by Network Toolkit. Do not retain this report longer than needed for approved licensing support.</div></main></body></html>
"@ | Set-Content -Path $path -Encoding UTF8

    $runIdentity = Resolve-NTKReportRunIdentity -InputObject $null -ComputerName $env:COMPUTERNAME
    $metadata = New-NTKReportMetadata -ReportType 'software-key-inventory' -Title 'Network Toolkit Software Key Report' -Format html -RunIdentity $runIdentity -Limitations @('Sensitive local licensing inventory; absence is not proof that a product is unlicensed.')
    [void](Register-NTKRunArtifact -RunIdentity $runIdentity -Path $path -ArtifactType 'software-key-report' -ReportMetadata $metadata)

    if(Get-Command Set-NTKArtifactMetadata -ErrorAction SilentlyContinue){
        Set-NTKArtifactMetadata -Path $path -Classification $(if($Reveal){'Sensitive'}else{'ClientEvidence'}) -RetentionDays $(if($Reveal){3}else{21}) | Out-Null
    }
    if(Get-Command Write-NTKSensitiveActionAudit -ErrorAction SilentlyContinue){
        Write-NTKSensitiveActionAudit -Action $(if($Reveal){'ExportUnmaskedSoftwareKeys'}else{'ExportMaskedSoftwareKeys'}) -ArtifactPath $path -Reason $Reason
    }
...
function Global:Invoke-SoftwareKeyFinder {
    param([switch]$Reveal,[switch]$ConfirmSensitiveAction,[switch]$IncludeApplicationRegistration,[int]$ApplicationScanBudgetSeconds=8,[string]$Reason='Interactive software licensing support')
    Clear-Host
    Write-Host ""
    Write-Host "SOFTWARE KEY FINDER" -ForegroundColor Cyan
    Write-Host "===================" -ForegroundColor DarkCyan
    Write-Host ""
    Write-Host "This performs a read-only local licensing check for Windows and legacy Office. Broad application registration scanning is optional." -ForegroundColor Yellow
    ...
    $keys = @(Get-NTKWindowsLicenseKeys) + @(Get-NTKOfficeLicenseKeys)
    if($IncludeApplicationRegistration){ $keys += @(Get-NTKApplicationLicenseEntries -MaxDurationSeconds $ApplicationScanBudgetSeconds) }
    $activation = @(Get-NTKRunObservationValue -Key 'cim.software-licensing-products' -Provider 'cim' -Collector { @(Get-NTKMicrosoftActivationInventory) })
    $reportPath = Export-NTKSoftwareKeyReport -RecoveredKeys $keys -ActivationInventory $activation -Reveal:$Reveal -ConfirmSensitiveAction:$ConfirmSensitiveAction -IncludedApplicationRegistration:$IncludeApplicationRegistration -Reason $Reason

    if($keys.Count -gt 0){
        Write-Host "Recoverable product keys:" -ForegroundColor Green
        $keys | Select-Object Product,@{Name='Key';Expression={if($Reveal){$_.Key}else{ConvertTo-NTKMaskedSensitiveValue $_.Key}}},Source | Format-Table -Wrap -AutoSize
    }
    else{
