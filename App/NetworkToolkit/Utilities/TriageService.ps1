...
function Global:Invoke-NTKTriageRun {
    ...
    try {
        ...
        elseif($Profile -eq "Quick"){
            [void]$warnings.Add([pscustomobject]@{severity="Info";message="Quick triage skipped portable tool auto-runs to keep one-click collection fast and reliable. Use Full Triage or Run Selected for external collectors."})
            $toolStatus = @()
        }
