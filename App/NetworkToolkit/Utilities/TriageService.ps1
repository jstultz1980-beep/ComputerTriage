...
function Global:New-NTKDefaultTriageManifest {
    $tools = @(
        ...
        @{name="WinAudit";id="winaudit";category="Triage";source="Parmavex";downloadUrl="http://parmavex.co.uk/winaudit.html";localPath=".\Triage\Tools\WinAudit";executables=@(@{name="WinAudit";path=".\Triage\Tools\WinAudit\WinAudit.exe";arguments="/r=gsoPxuTUeERNtnzDaIbMpmidcSArCOHG /f=ToolOutput\winaudit.html";outputFile="ToolOutput\winaudit.html";captureStdout=$false;autoRun=$true});required=$false;portable=$true;notes="Legacy optional inventory collector. Parmavex HTTPS certificate validation may fail; HTTP download page is currently reachable."}
        ...
        @{name="Wireshark Portable";id="wireshark";category="Triage";source="PortableApps.com";downloadUrl="https://portableapps.com/apps/internet/wireshark_portable";localPath=".\NetworkToolkit\ExternalTools\WiresharkPortable64";executables=@(@{name="Wireshark";path=".\NetworkToolkit\ExternalTools\WiresharkPortable64\WiresharkPortable64.exe";arguments="";outputFile="ToolOutput\wireshark.txt";captureStdout=$false;autoRun=$false;requiresConsent=$true});required=$false;portable=$true;notes="Packet captures are large and sensitive. Uses the toolkit ExternalTools Wireshark install instead of duplicating it under Triage\Tools."}
        @{name="LatencyMon";id="latencymon";category="Triage";source="Resplendence";downloadUrl="https://www.resplendence.com/latencymon";localPath=".\Triage\Tools\LatencyMon";executables=@(@{name="LatencyMon";path=".\Triage\Tools\LatencyMon\LatencyMon.exe";arguments="";outputFile="ToolOutput\latencymon.txt";captureStdout=$false;autoRun=$false});required=$false;portable=$true}
    )
