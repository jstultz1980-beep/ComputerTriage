...
$launcher = Join-Path $ToolkitRoot "NetworkToolkit-Core.ps1"
...
foreach($tool in $catalog){
    ...
    if($tool.External -and (Get-Command Resolve-NTKExternalTool -ErrorAction SilentlyContinue)){
        $resolved = Resolve-NTKExternalTool -Id $tool.External
...
$requiredCommands = @(
    ...
    "Get-NTKComputerFingerprint",
    "Save-NTKComputerFingerprint",
