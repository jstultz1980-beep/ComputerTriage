...
| --- | --- | --- | --- |
| Quick Diagnosis | One-click walk-up triage and fast target checks. | Run Quick Diagnosis, Open Latest Report, quick Ping/TCPing/Tracert/NSLookup/DNS/WHOIS, DISM/SFC repair after review, hardware shortcuts. | This is the first-use tab. It should stay focused and should not become a dumping ground. |
...
| Infrastructure | DHCP, DNS, time, and local exposure checks. | DHCP Sleuth, DHCP Scope Inspector, DHCP Lease / Rogue Check, DNS Diagnostics, Time Sync Health, Reset Domain Time, Local Exposure Inspector. | DHCP tools are grouped here. This tab was previously labeled Services. |
...
| Wi-Fi | Wireless-specific checks. | Wi-Fi Issue Scan, Wi-Fi Status, Wi-Fi Profiles, Wi-Fi Networks. | Keep separate from Network because wireless problems have distinct profile/radio/authentication causes. |
...
| Robocopy | Plain-language robocopy builder. | Build Command, Copy Command, Preview Only, Run Copy. | Dedicated tab is justified because the builder is a real workflow, not just a launcher. |
